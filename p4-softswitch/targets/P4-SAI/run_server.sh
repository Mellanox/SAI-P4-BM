...
rm -rf logs/*.txt
sudo -v
sudo ip netns exec sw_net ./simple_switch -i 0@router_port1 -i 8@router_cpu_port --thrift-port 9091 --log-file logs/router_log --log-flush --notifications-addr ipc:///tmp/bmv2-router-notifications.ipc sai_router.json &
export ROUTER_PID=$!
