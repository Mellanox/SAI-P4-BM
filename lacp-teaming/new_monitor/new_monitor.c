  ...
  team_for_each_port(port, th) {
    ...
    if (team_is_port_changed(port)) {
      ...
      if (team_is_port_link_up(port)) {
        printf("add lag member %s (ifindex %d)\n", get_port_name(th, ifindex),
               ifindex);
        struct team_ifinfo *ifinfo = team_get_port_ifinfo(port);
        char ifname[15];
        unsigned int maxlen = 15;
        team_ifindex2ifname(th, team_get_ifinfo_master_ifindex(ifinfo), ifname,
                            maxlen);
        printf("master %s\n", ifname);
      } else {
        printf("remove lag member %s (ifindex %d)\n",
               get_port_name(th, ifindex), ifindex);
      }
    }
  }
  return 0;
...
static struct team_change_handler port_change_handler = {
    .func = port_change_handler_func, .type_mask = TEAM_PORT_CHANGE,
};
  ...
  team_for_each_option(option, th) {
    if (team_is_option_changed(option)) {
      ...
      if ((strcmp("enabled", team_get_option_name(option)) == 0) |
          (strcmp("user_linkup_enabled", team_get_option_name(option)) == 0) |
          (strcmp("user_linkup", team_get_option_name(option)) == 0)) {
        printf("\topt %s changed to %s. ifindex = %d\n",
               team_get_option_name(option),
               team_get_option_value_bool(option) ? "True" : "False", ifindex);

      } else {
        printf("\topt %s changed to %s. ifindex = %d\n",
               team_get_option_name(option),
               team_get_option_value_string(option), ifindex);
      }
    }
  }
