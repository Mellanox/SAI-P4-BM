...
// parsing attributes
  ...
  for (uint32_t i = 0; i < attr_count; i++) {
    ...
    case SAI_HOSTIF_TRAP_ATTR_PACKET_ACTION:
      ...
      break;
    default:
      (*logger)->warn("in parsing hostif_trap, the following attr was dumped: {}",attribute.id);
    }
  }
  ...
  *hostif_trap_id = hostif_trap->sai_object_id;
      ...
      && hostif_trap->trap_action == SAI_PACKET_ACTION_TRAP )
  {
    ...
    action_data.clear();
    match_params.clear();
    match_params.push_back(parse_exact_match_param(1652522221570, 6)); // dmac : 01-80-c2-00-00-02
    action_data.push_back(parse_param(hostif_trap->sai_object_id, 2)); // trap_id
    hostif_trap->handle_l2_trap = bm_client_ptr->bm_mt_add_entry(
        cxt_id, "table_l2_trap", match_params,
        "action_set_trap_id", action_data, options);  
    (*logger)->info("added l2 trap - lacp , trap_id: {}",hostif_trap->sai_object_id);

    action_data.clear();
    match_params.clear();
    match_params.push_back(parse_exact_match_param(hostif_trap->sai_object_id, 2)); // trap_id 
    hostif_trap->handle_trap_id = bm_client_ptr->bm_mt_add_entry(
        cxt_id, "table_trap_id", match_params,
        "action_trap_to_cpu", action_data, options);  
    (*logger)->info("added LACP trap to cpu, trap_id: {}",hostif_trap->sai_object_id);
  } 
  else{
    (*logger)->warn("unsupported trap requested, trap type is: {}, trap_action is: {} \n ",hostif_trap->trap_type,hostif_trap->trap_action);
  }
}
...
sai_status_t sai_adapter::remove_hostif_trap(sai_object_id_t hostif_trap_id) {
  ...
  HostIF_Trap_obj *hostif_trap = switch_metadata_ptr->hostif_traps[hostif_trap_id];
  if (hostif_trap->trap_type == SAI_HOSTIF_TRAP_TYPE_LACP){
    try {
    ...
    (*logger)->warn("unsupported remove trap requested, trap type is: {}",hostif_trap->trap_type); 
  }
