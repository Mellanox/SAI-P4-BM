    ...
    for(uint32_t i = 0; i < thrift_attr_list.size(); i++, it++) {
        ...
        switch (attribute.id) {
            ...
            case SAI_FDB_ENTRY_ATTR_PACKET_ACTION:
                ...
                break;
            default:
                logger->error("--> while parsing fdb_attr: attribute.id = {} was dumped in sai_cpp_server",attribute.id); 
                break;
        }
    }
}
...
sai_fdb_entry_t  parse_thrift_fdb_entry(const sai_thrift_fdb_entry_t thrift_fdb_entry){
      ...
      for(uint32_t i = 0; i < thrift_attr_list.size(); i++, it++) {
          ...
          switch (attribute.id) {
              case SAI_VLAN_ATTR_VLAN_ID:
                ...
                break;
              default:
                logger->error("--> while parsing vlan_attr: {} was dumped in sai_cpp_server", attribute.id); 
                break;
          }
      }
  }
      ...
      for(uint32_t i = 0; i < thrift_attr_list.size(); i++, it++) {
          ...
          switch (attribute.id) {
              ...
              case SAI_VLAN_MEMBER_ATTR_VLAN_TAGGING_MODE:
                ...
                break; 
              default:
                logger->error("--> while parsing vlan_member_attr: {}  was dumped in sai_cpp_server" ,attribute.id); 
                break;
          }
      }
  }
  ...
  sai_thrift_object_id_t sai_thrift_create_switch(const std::vector<sai_thrift_attribute_t> & thrift_attr_list) {
    ...
    // sai_attribute_t *attr = (sai_attribute_t*) malloc(sizeof(sai_attribute_t) * thrift_attr_list.size());
    logger->info("sai_thrift_create_switch - currently only initiated on server constructor");
  }
  ...
  sai_thrift_object_id_t sai_thrift_get_port_id_by_front_port(const std::string& port_name) {
    ...
    //logger->info("--> freeing port_list_object_attribute.value.objlist.list");
...
int main(int argc, char **argv) {
  ...
  logger->info("creating server for SAI on port %d ", sai_port);
