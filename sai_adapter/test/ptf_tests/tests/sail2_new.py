...
class L21DLagTest(sai_base_test.ThriftInterfaceDataPlane):
    def runTest(self):
        ...
        lag_member1 = sai_thrift_create_lag_member(self.client, port1, lag)
        ...
        lag_member4 = sai_thrift_create_lag_member(self.client, port4, lag)
        ...
        # Create Bridge ports
        bridge_port_type = SAI_BRIDGE_PORT_TYPE_SUB_PORT
        self.client.sai_thrift_remove_bridge_port(bridge_port0)
        self.client.sai_thrift_remove_bridge_port(bridge_port1)
        ...
        bridge_port1 = sai_thrift_create_bridge_port(self.client, bridge_port_type, lag, vlan_id, bridge)

        # Create FDB Entries:
        mac_action = SAI_PACKET_ACTION_FORWARD
        fdb_entry_type = SAI_FDB_ENTRY_TYPE_STATIC
        fdb_bridge_type = SAI_FDB_ENTRY_BRIDGE_TYPE_1D
        sai_thrift_create_fdb(self.client, mac1, fdb_bridge_type, None, bridge, bridge_port0, mac_action, fdb_entry_type)
        sai_thrift_create_fdb(self.client, mac2, fdb_bridge_type, None, bridge, bridge_port1, mac_action, fdb_entry_type)
        ...
        try:
            ...
            for ip_id in [101,103,105,107]:
                ...
                verify_packets_any(self, pkt, [hw_port1, hw_port3, hw_port4])
        finally:
            sai_thrift_delete_fdb(self.client, mac1, None, bridge_type, bridge)
            sai_thrift_delete_fdb(self.client, mac2, None, bridge_type, bridge)
            vlan_id = 1
            ...
            self.client.sai_thrift_remove_bridge_port(bridge_port1)
            self.client.sai_thrift_remove_bridge(bridge)
            bridge_port0 = sai_thrift_create_bridge_port(self.client, bridge_port_type, port0, None, default_bridge)
            bridge_port1 = sai_thrift_create_bridge_port(self.client, bridge_port_type, port1, None, default_bridge)
            ...
            br_port_list[port0] = bridge_port0
            br_port_list[port1] = bridge_port1
